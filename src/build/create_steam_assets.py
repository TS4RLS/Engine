...
drifts out of sync with the app's own branding again. This is the single
source of truth for these images -- the Website repo has no copy of its own,
its steam.html page's previews just hotlink these files' raw GitHub URLs.
...
from src.cli import cli_colors  # noqa: E402
...
def _load_wordmark_parts():
    ...
    ("TS4RLS") and subtitle ("The Sims 4 Random Loading Screen") images so
...
def _pattern_layer(size, icon, tile_size, opacity, spacing_factor=1.35) -> Image.Image:
    ...
    (brick-style) grid -- smaller tiles and tighter spacing than the
    previous art's sparse, oversized icons, so it reads as an actual
    background pattern rather than a few stray logos."""
...
def _paste_centered(base: Image.Image, layer: Image.Image, cx: int, cy: int):
...
def _paste_stacked_wordmark(canvas, title, subtitle, target_subtitle_width, cx, top_y, gap=10):
...
def build_cover(icon: Image.Image, title, subtitle) -> Image.Image:
    size = (600, 900)
...
def build_background(icon: Image.Image) -> Image.Image:
    size = (3840, 1240)
    canvas = _textured_background(size, tile_size=110, opacity=0.14, icon=icon)
    return canvas.convert("RGB")


def build_logo(icon: Image.Image, title, subtitle) -> Image.Image:
...
def build():
    print(cli_colors.banner(
        "TS4RLS - Steam Artwork Generator",
        "Regenerating assets/steam/ from icon.png + logo.png",
    ))
